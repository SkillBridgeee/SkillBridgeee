...
class FakeListingRepository(private val initial: List<Listing> = emptyList()) : ListingRepository {
    ...
    private val listings = mutableMapOf<String, Listing>().apply {
    ...
    override suspend fun getListing(listingId: String): Listing =
        synchronized(listings) { listings[listingId] ?: throw NoSuchElementException("Listing $listingId not found") }
    ...
    override suspend fun updateListing(listingId: String, listing: Listing) {
        synchronized(listings) {
            if (!listings.containsKey(listingId)) throw NoSuchElementException("Listing $listingId not found")
    ...
    override suspend fun searchByLocation(location: Location, radiusKm: Double): List<Listing> =
        synchronized(listings) {
            // best-effort: if a listing exposes a location-like field, compare equals; otherwise return all
