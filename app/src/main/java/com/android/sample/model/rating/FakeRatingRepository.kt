...
class FakeRatingRepository(private val initial: List<Rating> = emptyList()) : RatingRepository {
    ...
    private val ratings = mutableMapOf<String, Rating>().apply {
        initial.forEach { put(getIdOrGenerate(it), it) }
    }

    override fun getNewUid(): String = UUID.randomUUID().toString()

    override suspend fun getAllRatings(): List<Rating> = synchronized(ratings) { ratings.values.toList() }

    override suspend fun getRating(ratingId: String): Rating =
        synchronized(ratings) { ratings[ratingId] ?: throw NoSuchElementException("Rating $ratingId not found") }
    ...
    override suspend fun addRating(rating: Rating) {
        synchronized(ratings) {
            ratings[getIdOrGenerate(rating)] = rating
        }
    }
    ...
    override suspend fun getTutorRatingsOfUser(userId: String): List<Rating> =
        synchronized(ratings) {
            // Heuristic: ratings for tutors related to listings owned by this user OR ratings targeting the user.
            ratings.values.filter { r ->
        ...
        try {
            ...
            for (name in names) {
                ...
                val method = obj.javaClass.methods.firstOrNull { m ->
                    m.parameterCount == 0 && (m.name.equals(getter, true) || m.name.equals(name, true) || m.name.equals(isMethod, true))
                }
                if (method != null) {
                    try {
