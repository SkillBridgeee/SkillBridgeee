 ...
 *
 * This sealed type keeps presentation concerns separate from the repository/domain layer:
 * the ViewModel is responsible for mapping domain models into [BookingCardUi] and emitting
 * the correct [MyBookingsUiState] variant.
 */
sealed class MyBookingsUiState {
     ...
     * - The UI should render these items (for example via `LazyColumn` using each item's `id`
     *   as a stable key).
     */
     ...
     *
     * UI: show an empty-state illustration/message and possible call-to-action (e.g., "Book a tutor").
     */
     ...
     *
     * UI: display the message and provide a retry or support action.
     * The message may be generic for end-users or more detailed for logging depending on how the
     * ViewModel formats it.
     */
