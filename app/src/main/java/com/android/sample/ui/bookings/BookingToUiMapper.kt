...
class BookingToUiMapper(private val locale: Locale = Locale.getDefault()) {
            ...
            possibleNames.forEach { name ->
                ...
                val method =
                    b.javaClass.methods.firstOrNull { m ->
                        m.parameterCount == 0 && (m.name.equals(getter, ignoreCase = true) || m.name.equals(name, ignoreCase = true))
                    }
                if (method != null) {
                    try {
    ...
    private fun findValueOn(obj: Any, possibleNames: List<String>): Any? {
        try {
            ...
            possibleNames.forEach { name ->
                ...
                val field =
                    cls.declaredFields.firstOrNull { f -> f.name.equals(name, ignoreCase = true) }
                if (field != null) {
                    try {
                        field.isAccessible = true
                        val v = field.get(obj)
    ...
    private fun safeNestedStringProperty(b: Booking, parentNames: List<String>, childNames: List<String>): String? {
    ...
    fun map(b: Booking): BookingCardUi {
        ...
        val tutorName =
            safeStringProperty(
                b,
                listOf("tutorName", "tutor", "listingCreatorName", "creatorName")
            )
                ?: safeNestedStringProperty(b, listOf("tutor", "listingCreator", "creator"), listOf("name", "fullName", "displayName", "tutorName", "listingCreatorName"))
                ?: safeNestedStringProperty(b, listOf("associatedListing", "listing", "listingData"), listOf("creatorName", "listingCreatorName", "creator", "ownerName"))
                ?: safeStringProperty(b, listOf("listingCreatorId", "creatorId")) ?: "—"

        val subject =
            safeStringProperty(b, listOf("subject", "title", "lessonSubject", "course"))
                ?: safeNestedStringProperty(b, listOf("associatedListing", "listing", "listingData"), listOf("subject", "title", "lessonSubject", "course"))
        ...
        val dateLabel = try { dateFormat.format(start) } catch (_: Throwable) { "" }
