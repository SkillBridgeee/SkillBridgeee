            ...
            if (uiState.isLoading) {
                ...
                LazyColumn(
                    ...
                    items(uiState.messages.reversed()) { message ->
                        MessageBubble(message = message, isCurrentUser = message.sentFrom == currentUserId)
                    }
                }
            }
        }
...
fun MessageBubbleCurrentUserPreview() {
    MessageBubble(
        message = Message(content = "This is a message from the current user."),
        isCurrentUser = true
    )
}
...
fun MessageBubbleOtherUserPreview() {
    MessageBubble(
        message = Message(content = "This is a message from another user."),
        isCurrentUser = false
    )
}
...
fun MessageScreenPreview() {
    ...
    MaterialTheme {
        MessageScreen(viewModel = viewModel, currentUserId = "user1")
    }
}
