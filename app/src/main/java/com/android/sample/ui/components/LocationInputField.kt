...
/**
 * A composable input field for searching and selecting a location (TextField version with custom
 * styling).
 *
 * Displays a TextField that allows the user to enter a location name or address, along with an
 * optional dropdown list of location suggestions. This version accepts custom shape and colors.
 *
 * When the user types into the text field, [onLocationQueryChange] is triggered to update the
 * search query, and the dropdown menu appears with matching [locationSuggestions]. Selecting an
 * item from the dropdown triggers [onLocationSelected] and closes the menu.
