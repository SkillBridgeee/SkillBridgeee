...
class MyBookingsRobolectricExtraTest {
  ...
  private fun setWithItems(items: List<BookingCardUi>) {
    ...
    compose.setContent {
      SampleAppTheme { MyBookingsScreen(vm = vm, navController = rememberNavController()) }
    }
  }
  ...
  fun details_buttons_count_matches_cards_and_click_works() {
    ...
    compose
        .onAllNodesWithTag(BOOKING_DETAILS_BUTTON)
  ...
  fun rating_is_clamped_between_0_and_5() {
    val clampedItems =
        listOf(
            BookingCardUi(
                "hi", "alice", "Piano", "$10/hr", "1hr", "01/01/2026", -1, 0), // -> 0 stars
            BookingCardUi(
                "lo", "bob", "Guitar", "$20/hr", "2hrs", "02/01/2026", 7, 99) // -> 5 stars
            )
  ...
  fun avatar_initial_is_uppercased_first_letter() {
    val single = listOf(BookingCardUi("x", "zoe l.", "Math", "$15/hr", "1hr", "03/01/2026", 3, 10))
    setWithItems(single)
