...
class SubjectListViewModelTest {
    ...
    private fun profile(
        ...
        name = name,
        description = desc,
        tutorRating = RatingInfo(rating, total)
    )

    private fun skill(userId: String, s: String) = Skill(
        userId = userId,
    ...
    private val defaultRepo = FakeRepo(
    ...
    fun topTutors_respects_tieBreakers_and_limit() = runTest {
        ...
        val repo = FakeRepo(
            profiles = listOf(A, X, Y), // A has 4.9; X and Y tie on rating & totals -> name tie-break
            skills = emptyMap()
        )
