...
class SubjectListScreenTest {
    ...
    private fun skill(s: String) = Skill(
        userId = "",
    ...
    private fun setContent(onBook: (Profile) -> Unit = {}) {
        ...
        composeRule.setContent {
            MaterialTheme {
                SubjectListScreen(viewModel = vm, onBookTutor = onBook)
            }
        }
        // Wait until refresh() finishes and lists are shown
        composeRule.waitUntil(timeoutMillis = 5_000) {
            // top tutors shows up when loaded
            composeRule.onAllNodesWithTag(SubjectListTestTags.TOP_TUTORS_SECTION).fetchSemanticsNodes().isNotEmpty()
        }
    }
    ...
    fun clickingBook_callsCallback() {
        ...
        composeRule.onAllNodesWithTag(SubjectListTestTags.TUTOR_BOOK_BUTTON)
            .onFirst()
            .performClick()
    ...
    fun searchFiltersList_visually() {
        ...
        composeRule.onNodeWithTag(SubjectListTestTags.SEARCHBAR)
            .performTextInput("Nora")
