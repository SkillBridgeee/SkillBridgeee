...
class ListingScreenTest {
  ...
  @Test
  fun listingScreen_errorDialog_displaysWhenBookingFails() {
    val listingRepo = FakeListingRepo(sampleProposal)
    ...
    // Complete booking with auto-filled dates (will fail)
    compose.onNodeWithTag(ListingScreenTestTags.BOOK_BUTTON).performClick()
    compose.waitForIdle()

    compose.waitUntil(5_000) {
      compose
          .onAllNodesWithTag(ListingScreenTestTags.CONFIRM_BOOKING_BUTTON, useUnmergedTree = true)
          .fetchSemanticsNodes()
          .isNotEmpty()
    }
    ...
    compose.onNodeWithText("Booking Error").assertIsDisplayed()
  }

  @Test
  fun listingScreen_errorDialog_okButton_clearsError() {
    val listingRepo = FakeListingRepo(sampleProposal)
    val profileRepo = FakeProfileRepo(mapOf("creator-456" to sampleCreator))
    val bookingRepo = FakeBookingRepo(shouldSucceed = false)
    val vm = ListingViewModel(listingRepo, profileRepo, bookingRepo)
    ...
    compose.onNodeWithTag(ListingScreenTestTags.ERROR_DIALOG).assertDoesNotExist()
  }
